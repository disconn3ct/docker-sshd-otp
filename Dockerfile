FROM ubuntu:trusty
RUN apt-get update && apt-get upgrade -y && apt-get install -y ssh rsyslog fail2ban openssh-server openssh-client supervisor python-pyinotify libpam-google-authenticator && apt-get clean
